...
function serializeOrder(order: Order) {
  return {
    ...
    items: order.items.map(item => ({
      ...
      sellerAmount: item.sellerAmount || null,
    })),
    buyer: order.buyer,
  ...
  try {
    ...
    if (isStalePendingIntouch) {
      try {
        ...
        // Audit log: stale-pending status check performed
        await logPaymentEvent("intouchpay_status_checked", {
          orderId: params.id,
          ...
          payload: { staleCheckMs: STALE_CHECK_MS },
        })
      } catch (error) {
