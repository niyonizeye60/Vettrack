...
interface InseminationManagerProps {
...
export default function InseminationManager({ farmerId, can, showHeader = true }: InseminationManagerProps) {
  ...
  // A delegated vet is the vet who performed the procedure, so the field starts as
  // - and stays - their own name. The farmer instead picks from the roster below.
  const [vetName, setVetName] = useState(can.isDelegate ? can.currentUserName : "")
  ...
  useEffect(() => {
    async function init() {
      ...
      const animalsRes = await fetch(`/api/farm-animals?farmerId=${farmerId}&module=insemination`)
  ...
  const fetchRecords = async (id: string) => {
    const res = await fetch(`/api/insemination?farmerId=${id}`)
  ...
  const resetForm = () => {
    ...
    setInjectionTime(""); setExpectedBirthDate(""); setDeliveredBabies("")
    setVetName(can.isDelegate ? can.currentUserName : ""); setVetOrigin("")
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.text(exportAnimalName ? `${t('farmer.inseminationReportTitle')} — ${exportAnimalName}` : t('farmer.inseminationReportTitle'), 15, 18)
      ...
      for (let page = 1; page <= totalPages; page++) {
        ...
        doc.text(`Generated by: ${can.currentUserName || "Unknown"}`, 15, pageHeight - 7)
        doc.text(`© ${new Date().getFullYear()} NTDM Vettrack. All rights reserved`, pageWidth / 2, pageHeight - 7, { align: "center" })
          ...
          <Card className="border border-gray-200 shadow-sm">
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* Vet Name - for the farmer, a roster pick; for a delegated vet, themselves.
                    Shown read-only rather than hidden so the vet can see the attribution
                    that will appear on the farmer's record, and rather than disabled so it
                    does not read as a permission they are missing. */}
                <div className="space-y-1">
                  ...
                  {can.isDelegate ? (
                    <>
                      ...
                      <div className="flex h-10 items-center gap-2 rounded-md border border-gray-200 bg-gray-50 px-3 text-sm text-gray-900">
                        {editRecord ? (
                          <span className="font-medium">{vetName || <span className="font-normal text-gray-400">—</span>}</span>
                        ) : (
                          <span className="font-medium">{can.currentUserName}</span>
                        )}
                        {(editRecord ? vetName : can.currentUserName) === can.currentUserName && (
                      ...
                      <TableRow key={r._id}>
                        ...
                        <TableCell>
                          <div className="flex gap-1">
                            {/* Re-inseminating writes to the previous row, so it needs update. */}
                            {r.deliveredBabies == null && canModify(can, "update", r) && (
