...
export default function DiseaseManager({ farmerId, can, showHeader = true }: DiseaseManagerProps) {
  ...
  // A delegated vet is the vet attending this case, so the field starts as - and
  // stays - their own name. The farmer instead picks from the roster below.
  const [veterinarianName, setVeterinarianName] = useState(can.isDelegate ? can.currentUserName : "")
  ...
  // Nothing to do on the entry tabs without write access.
  const showRecordTab = can.create || can.update
  // The entry tabs double as edit forms, so a user with update-but-not-create still
  // needs them - but must not be able to submit a brand new record or dose.
  const canSubmitForm = editRecord ? can.update : can.create
  const canSubmitDose = editDose ? can.update : can.create
  ...
  useEffect(() => {
    async function init() {
      ...
      // Only the farmer picks a vet from the roster. Skipping this for a delegated
      // vet avoids pulling every doctor's contact details into a portal that has no
      // use for them.
      const [animalsRes, doctorsData] = await Promise.all([
        fetch(`/api/farm-animals?farmerId=${farmerId}&module=health`),
        can.isDelegate ? Promise.resolve([]) : getDoctorsList(),
      ])
      const animalsData = animalsRes.ok ? await animalsRes.json() : []
      setAnimals(Array.isArray(animalsData) ? animalsData : [])
      setDoctors(doctorsData)
      await Promise.all([fetchRecords(farmerId), fetchDoses(farmerId)])
      setLoading(false)
    }
    init()
  }, [farmerId])
  ...
  const fetchRecords = async (id: string) => {
    const res = await fetch(`/api/diseases?farmerId=${id}`)
    const data = await res.json()
    setRecords(Array.isArray(data) ? data : [])
  }

  const fetchDoses = async (id: string) => {
    const res = await fetch(`/api/treatment-doses?farmerId=${id}`)
    const data = await res.json()
  ...
  const resetForm = () => {
    ...
    setStatus("Active"); setVetOrigin(""); setNotes("")
    setVeterinarianName(can.isDelegate ? can.currentUserName : "")
  ...
  const exportToPDF = async () => {
    ...
    try {
      ...
      doc.text(t('farmer.diseaseManagementReportTitle'), 15, 18)
      ...
      doc.text(`Generated by: ${can.currentUserName || 'Unknown'}`, 15, 66)
      ...
      for (let page = 1; page <= totalPages; page++) {
        ...
        doc.text(`Generated by: ${can.currentUserName || 'Unknown'}`, 15, ph - 7)
        doc.text(`© ${new Date().getFullYear()} NTDM Vettrack. All rights reserved`, PW / 2, ph - 7, { align: 'center' })
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Record and Doses are entry tabs - hidden for a view-only user, who still gets
          the full History and Reports. Doses ride on the same health permission. */}
      <Tabs defaultValue={showRecordTab ? "record" : "history"}>
        <TabsList className={`grid w-full max-w-2xl ${showRecordTab ? "grid-cols-4" : "grid-cols-2"}`}>
          {showRecordTab && (
          ...
          <Card className="border border-gray-200 shadow-sm">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {/* For the farmer, a roster pick; for a delegated vet, themselves. Shown
                    read-only rather than hidden so the vet can see the attribution that
                    will appear on the farmer's record, and rather than disabled so it does
                    not read as a permission they are missing. */}
                <div className="space-y-1">
                  <label className="text-sm font-medium text-gray-700">
                    {t('farmer.veterinarian')}{" "}
                    {!can.isDelegate && <span className="text-gray-400 text-xs">({t('common.optional')})</span>}
                  </label>
                  {can.isDelegate ? (
                    <>
                      {/* Mirror exactly what the server will store: your own name on a new
                          record, the untouched original when editing an existing one. */}
                      <div className="flex h-10 items-center gap-2 rounded-md border border-gray-200 bg-gray-50 px-3 text-sm text-gray-900">
                        {editRecord ? (
                          <span className="font-medium">{veterinarianName || <span className="font-normal text-gray-400">—</span>}</span>
                        ) : (
                          <span className="font-medium">{can.currentUserName}</span>
                        )}
                        {(editRecord ? veterinarianName : can.currentUserName) === can.currentUserName && (
          ...
          <div className="space-y-6">
            ...
            <Card className="border border-gray-200 shadow-sm">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  <Button onClick={handleDoseSubmit} disabled={savingDose || !canSubmitDose} className="bg-orange-500 hover:bg-orange-600 text-white rounded-lg px-6">
